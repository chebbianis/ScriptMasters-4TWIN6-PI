...
export const verifyFace = async (req, res) => {
  try {
    ...
    // Vérifier si l'utilisateur a enregistré un visage
    if (!user.faceImagePath) {
      return res.status(400).json({
        success: false,
        message: "Cet utilisateur n'a pas configuré d'authentification faciale"
      });
    }

    // Dans un système réel, nous ferions ici la comparaison des visages
    // Pour ce prototype, supposons que la vérification est réussie
