...
export const getProjectNamesQueryFn = async (workspaceId: string): Promise<{
  projects: { _id: string; name: string }[];
}> => {
  if (!workspaceId) {
    console.error("Workspace ID is required for fetching projects");
    return { projects: [] };
  }

  console.log(`Fetching projects for workspace: ${workspaceId}`);
  const response = await API.get(`/project/names?workspaceId=${workspaceId}`);
