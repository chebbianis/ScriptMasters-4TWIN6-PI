...
import { createProjectMutationFn, getDevelopersQueryFn, createNotification } from "@/lib/api";
...
export default function CreateProjectForm({
  ...
  const { mutate, isLoading: isMutationLoading } = useMutation({
    ...
    onSuccess: async (data, variables, context) => {
      ...
      if (data && project && form.getValues("users")?.length > 0) {
        const selectedDevelopers = form.getValues("users");
        console.log("Développeurs sélectionnés:", selectedDevelopers); // Ajouter un log
        selectedDevelopers.forEach(async (userId) => {
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <select {...field} className="mt-1 block w-full border rounded p-2">
